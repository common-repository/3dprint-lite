...
class p3dlitePrinters_List extends WP_List_Table {
	...
	public function __construct() {
		...
		parent::__construct( [
			'singular' => esc_html__( 'Printer', '3dprint-lite' ), //singular name of the listed records
			'plural'   => esc_html__( 'Printers', '3dprint-lite' ), //plural name of the listed records
	...
	public static function apply_filter($sql) {
		...
		if (isset($_REQUEST['printer_text']) && strlen($_REQUEST['printer_text'])>0) {
			...
			$printer_text = $wpdb->esc_like( sanitize_text_field( wp_unslash(trim($_REQUEST['printer_text']))));
	 ...
	 * @return mixed
	 */
	public function get_p3dprinters( $per_page = 5, $page_number = 1 ) {
	...
	public function no_items() {
		esc_html_e( 'No printers avaliable.', '3dprint-lite' );
	}
...
#				return print_r( $item, true ); //Show the whole array for troubleshooting purposes
	...
	function column_name( $item ) {
		...
		$actions = [
			'edit' => sprintf( '<a href="?page=%s&action=%s&printer=%s&_wpnonce=%s">'.esc_html__("Edit", '3dprint-lite').'</a>', 'p3dlite_printers', 'edit', absint( $item['id'] ), $edit_nonce ),
			'clone' => sprintf( '<a href="?page=%s&action=%s&printer=%s&_wpnonce=%s">'.esc_html__("Clone", '3dprint-lite').'</a>', 'p3dlite_printers', 'clone', absint( $item['id'] ), $clone_nonce ),
			'delete' => sprintf( '<a href="?page=%s&action=%s&printer=%s&_wpnonce=%s">'.esc_html__("Delete", '3dprint-lite').'</a>', 'p3dlite_printers', 'delete', absint( $item['id'] ), $delete_nonce )
		];
...
class p3dliteP_Plugin {
	...
	public function plugin_settings_page() {
		...
		?>
		<div class="wrap">
			<h2><?php esc_html_e('Printers', '3dprint-lite');?> </h2>

			<div id="poststuff p3d-lite-poststuff">
				<button class="button-secondary" type="button" onclick="location.href='<?php echo esc_url(admin_url( 'admin.php?page=p3dlite_printers&action=add' ));?>'"><b><?php esc_html_e('Add Printer', '3dprint-lite');?></b></button>
				<div id="post-body" class="metabox-holder columns-2">
					<div id="post-body-content">
						<div class="meta-box-sortables ui-sortable p3d-lite-table">
							<form name="printer_form" method="post">
								<?php esc_html_e('Search in name or description', '3dprint-lite');?>: <input name="printer_text" value="<?php if(isset($_REQUEST['printer_text'])) echo esc_attr(esc_html($_REQUEST['printer_text']));?>">&nbsp;
